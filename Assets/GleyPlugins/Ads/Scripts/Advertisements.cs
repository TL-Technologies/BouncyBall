...
public class Advertisements : MonoBehaviour
{
    ...
    public void Initialize()
    {
        if (initialized == false)
        {
            ...
            try
            {
                if (adSettings.advertiserSettings.First(cond => cond.advertiser == SupportedAdvertisers.Facebook).useSDK)
                {
            ...
            {
                Debug.Log("Your package was updated recently, go to Window->Gley->Ads and press Save Button to save the changes");
            }
